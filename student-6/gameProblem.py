...
class GameProblem(SearchProblem):
	...
	if(action == 'North'):
		...
		bateria=state[2]
		nueva_bateria=state[2]-self.getAttribute(nueva_posicion,"bateria")

		if nueva_bateria > 100:
			nueva_bateria=100
		...
		state_final=(nueva_posicion,nuevas_metas,nueva_bateria)
	...
	coste=self.getAttribute(state2[0],"cost")
    ...
    def setup (self):
        initial_state = (self.AGENT_START, tuple(self.POSITIONS.get("goal")), 100)
        final_state= (self.AGENT_START, tuple(),0)
        algorithm= astar
