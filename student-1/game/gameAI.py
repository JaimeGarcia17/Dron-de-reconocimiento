...
# Map related vars
# it can be Random or File (map_file)
#init_map = configuration['mapa']['type'].lower()
#save_map_p = (configuration['mapa']['type'].lower() == 'random')
#map_file = configuration['mapa']['file']
...
def init_mygame():
    ...
    if configuration['type'] == 'random':
        map = mapas.create_map(configuration, state, configuration['debug'])
    else:
        map, configuration = mapas.read_map(configuration)
        
    # display
    if configuration['showState']:
        screen_size = [configuration['map_size'][0] * tile_size, configuration['map_size'][1] * tile_size + text_size]
    else:
        screen_size = [configuration['map_size'][0] * tile_size, configuration['map_size'][1] * tile_size + 30]
    ...
    if configuration['save']:
        with open(configuration['file'], 'w') as f:
            f.write(mapas.printable_map(map, configuration, False))

    mapas.print_map(map, configuration, images, screen, state, tile_size, fsm_state, configuration['debug'],"Running search")
...
def do_move_agent(state, mapa, new_pos):
    ...
    location=mapa[new_pos[0]][new_pos[1]][0]
    if location in locationEffects.keys():
        f = locationEffects[location]
    else:
        f = locationEffects['default']
        
    return f(state,mapa,new_pos)        
...
def init_game():
    ...
    while not done:
        ...
        if ai201718 and len(aiPlan)>0 and not state['inPause']:
            ...
            # if not (tuple(new_pos) == expectedNextPosition):
            #     print ("main loop (ERROR): expected next Pos is {0}, moving agent to {1}".format(expectedNextPosition,new_pos))
            #     print ("   Contents of new position is {0}".format(mapa[new_pos[0]][new_pos[1]]))
            #     print ("   Contents of expected new position is {0}".format(mapa[expectedNextPosition[0]][expectedNextPosition[1]]))
            #     done=True
            #     print (mapas.printable_map(mapa,configuration,True))
            #     continue
        ...
        # --- Drawing code
        mapas.print_map(mapa, configuration, images, screen, state, tile_size, fsm_state, configuration['debug'],show_text=displayText)
    ...
    state['inPause']=True
    mapas.print_map(mapa, configuration, images, screen, state, tile_size, fsm_state, configuration['debug'],show_text=aiMapText)
    while state['inPause']:
        done, state = read_events(configuration, state)

    #print("Points: "+str(state['points'])+" Eats: "+str(state['eats'])+" Knives: "+str(state['knives'])+" Knives (bad): "+str(state['bad_knives'])+
    #      " Bad Eats: "+str(state['bad_eats'])+" Moves: "+str(state['moves'])+" Plans: "+str(state['plans'])+
    #      " Actions: "+str(state['actions'])+" Winner: "+winner)
    pygame.quit()
